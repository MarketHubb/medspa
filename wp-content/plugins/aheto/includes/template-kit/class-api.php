...
class API {
	...
	private $url = 'https://import.aheto.co/wp-json';
	...
	public function check_url(){
		...
		$check_url = strpos($string_content, "import.aheto.co");

		if(count($content)){
			return ( false === $check_url ) ? '' : 'https://import.aheto.co/wp-json';
		}
		return 'https://import.aheto.co/wp-json';
	}
...
//		}
	...
	public function get_demodata($endpoint, $callback = false, $cache = true ){
		return $this->get_data($endpoint, $callback, $cache);
	}
	 ...
	 * @return mixed
	 */
	private function get_data( $endpoint, $callback = false, $cache = true ) {
		...
		if (file_exists($saved_data) and file_get_contents($saved_data))
		{

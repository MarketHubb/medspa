<?php
	...
	function aheto_dynamic_css_array ( $settings )
	{
		...
		if ( !empty( $settings[ 'links' ] ) ) {
			...
			$link = $settings[ 'links' ];
			$link_hover = $settings[ 'links' ][ 'color_hover' ];
...
//				$css['global'][$element]['font-style'] = $value['font-style'];
	...
	function aheto_buttons ( &$css, $settings )
	{
		...
		if (isset($settings['button']))
			...
			$button = ""; 
		if (isset($settings['button_primary']))	
			$button_primary = $settings[ 'button_primary' ];
			
		$button_primary_large = $settings[ 'button_primary_large' ];
		$button_primary_small = $settings[ 'button_primary_small' ];

		$button_dark = $settings[ 'button_dark' ];
		$button_dark_large = $settings[ 'button_dark_large' ];
		$button_dark_small = $settings[ 'button_dark_small' ];

		$button_light = $settings[ 'button_light' ];
		$button_light_large = $settings[ 'button_light_large' ];
		$button_light_small = $settings[ 'button_light_small' ];


		$button_small = $settings[ 'button_small' ];
		$button_large = $settings[ 'button_large' ];

		$button_inline = $settings[ 'button_inline' ];
		$button_inline_dark = $settings[ 'button_inline_dark' ];
		$button_inline_light = $settings[ 'button_inline_light' ];

		$button_video = $settings[ 'button_video' ];
		$button_video_small = $settings[ 'button_video_small' ];
		$button_video_large = $settings[ 'button_video_large' ];
...
//		'tablet' => '@media (max-width: 991px)',
		...
		foreach ( [ $button_primary, $button_dark, $button_light, $button_primary_large, $button_primary_small, $button_dark_large, $button_dark_small, $button_light_large, $button_light_small ] as $button ) {
			...
			aheto_add_props ( $css[ 'global' ][ aheto_implode ( $elements ) ], Sanitize ::typography ( $example[ 'font' ] ) );
			...
			if ( is_array ( $example[ 'tablet_padding' ] ) && !empty( $example[ 'tablet_padding' ] ) ) {
				aheto_add_props ( $css[ '@media (max-width: 991px)' ][ aheto_implode ( $elements ) ], Sanitize ::spacing ( $example[ 'tablet_padding' ] ) );
			}
			if ( is_array ( $example[ 'mobile_padding' ] ) && !empty( $example[ 'mobile_padding' ] ) ) {
		...
		foreach ( [ $button_inline, $button_inline_dark, $button_inline_light ] as $button ) {
			$example = !empty( $button[ 'font' ] ) ? $button[ 'font' ] : $button[ 0 ][ 'font' ];
			$example_hover = $button[ 'font' ][ 'color_hover' ] ? $button[ 'font' ][ 'color_hover' ] : $button[ 0 ][ 'font' ][ 'color_hover' ];
			...
			aheto_add_props ( $css[ 'global' ][ aheto_implode ( $elements ) ], Sanitize ::typography ( $example ) );
...
//	);
	...
	function aheto_forms ( &$css, $settings ){
		...
		foreach ( [ $input, $textarea] as $item ) {
			...
			if ( !empty( $item[ 0 ][ 'color_placeholder' ] ) ) {
		...
		if ( !empty( $checkbox[ 0 ][ 'box_shadow' ] ) ) {
		...
		if ( !empty( $checkbox[ 0 ][ 'border_hover' ] ) ) {
